...
export class NewsArticlesRemoteService {
  ...
  async fetchArticles(): Promise<{
    ...
    const response_with_metadata = raw_response
      .map((r) => r.data.map((d) => ({ ...d, source: r.metadata })))
      .flat();
    return { data: response_with_metadata, errors: NewsSourcesReceiverService.selectErrors(raw_response) };
  }
}
